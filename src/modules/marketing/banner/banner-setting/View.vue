...
import { fetchBannerSettingDisplays, updateBannerSettingDisplay, multipleIdx } from './api'
...
export default {
  ...
  methods: {
    ...
    async fetchBannerSettingDisplays () {
      ...
      try {
        ...
        if (data?.code === repStatus.sussess) {
          ...
          this.displayType = data.data?.statistics?.displayType
        }
      } catch (error) {
        return false
      } finally {
    ...
    async handleSave () {
      ...
      try {
        const { data } = await updateBannerSettingDisplay({ displayType: this.displayType })
        if (data?.code === repStatus.sussess) {
    ...
    async changePosition () {
      this.ui.isSubmiting = true
      try {
        const resource = this.listBannerSettingDisplays.reduce((array, item, index) => {
