      ...
      <div class="banner__info">
        <div  class="card-item" v-for="(item, index) in ui.form" :key="index">
          ...
          <p v-if="item ==='status'" class="card-item__value">
            {{ bannerDetails.display === 0 ? $t('option.status.disable') : $t('option.status.active') }}
          </p>
          <!-- ------------------- -->
          <div v-else-if="item === 'targetType'" class="card-item__value">
            ...
            <p v-else-if="bannerDetails[item] === TARGET_TYPE.MRO">
              <span>{{ $t("option.targetType.hotelAdmin") }}</span>
            </p>
