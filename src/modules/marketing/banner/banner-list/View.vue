<template>
  <sa-section class="banner">
      ...
      <div class="banner__header">
        <div class="banner__header--search">
          <div class="mb-1em w-100">
            <el-input :placeholder="$t('breadcrumb.title')"
          ...
          <div>
            ...
            v-model="filter.provinceSn">
              <el-option
              v-for="(item, index) in dataProvinces"
              :key="index"
              :label="index === 0 ? $t(item.name) : item.name"
              :value="item.sn">
              </el-option>
            </el-select>
          </div>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <router-link v-if="scope.row.targetType === 10 && scope.row.targetSn" :to="{ name: 'createGroupPromotion', params: { promotionSn: scope.row.targetSn } }">
            {{ scope.row.targetName }}
          </router-link>
          <router-link v-else-if="scope.row.targetType === 1 && scope.row.targetSn && scope.row.targetName" :to="{ name: 'promotionDetails', params: { promotionSn: scope.row.targetSn } }">
            {{ scope.row.targetName }}
          </router-link>
      ...
      <el-table-column
        :label="$t('page.banner.displaySystem')"
        min-width="150"
      >
        <template slot-scope="scope">
          <p v-if="scope.row.type === 2">Web</p>
          <p v-else-if="scope.row.type === 1">App</p>
          <p v-else>Hotel Admin</p>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
import { TARGET_TYPE, repStatus } from '@/utils/const'
...
export default {
  ...
  methods: {
    getTargetType (targetType) {
      ...
      else if (targetType === TARGET_TYPE.MRO) return `${this.$t('option.targetType.hotelAdmin')}`
      else return ''
    },
    async fetchProvinces () {
      this.ui.isLoadingProvinces = true
      try {
        this.provinces = [{ name: 'option.all', sn: -1 }]
    ...
    setFixed () {
      return !this.ui.isLoading ? 'right' : null
    }
  }
}
</script>
...
<style lang="scss" scoped>
    .banner:deep(.el-table__body-wrapper) {
      width: calc(100% - 8px);
    }
    .banner:deep(.el-table__header-wrapper) {
      width: calc(100% - 8px);
    }
  .banner {
    &__button {
