...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        srcImage: [
          { required: true, message: this.$t('page.banner.requiredImagePath'), trigger: 'change' }
        ],
        srcImageBanner: [
          { required: true, message: this.$t('page.banner.requiredImageBanner'), trigger: 'change' }
        ]
      },
      ui: {
        isLoading: false,
  ...
  computed: {
    ...
    checkUploadImageBannerMRO: function () {
      if (this.srcImage) {
        return true
      } else {
        return false
    ...
    checkUploadImageAds: function () {
      if (this.srcImageBanner) {
        return true
      } else {
        return false
      }
    ...
    async getBannerDetails () {
      ...
      try {
        ...
        if (data?.code === repStatus.sussess) {
          ...
          this.srcImageBanner = this.formatImage(data.data.targetInfo)
