export default
{
  yes: 'Có',
  no: 'Không',
  all: 'Tất cả',
  selected: 'Đã chọn',
  allSelected: 'Chọn tất cả',
  public: 'Công khai',
  private: 'Riêng tư',
  hight: 'Chiều cao',
  weight: 'Chiều rộng',
  hotelStatus: {
    waiting: 'Chờ duyệt',
    displayed: 'Đã duyệt',
    contracted: 'Hợp đồng',
    trial: 'Dùng thử',
    terminated: 'Dừng hợp đồng',
    suspended: 'Treo',

    hotelAvailable: 'Có',
    hotelUnAvailable: 'Không'
  },

  bookingNotificationBy: {
    go2joyParterApp: 'Ứng dụng Go2Joy Partner',
    sms: 'Tin nhắn',
    email: 'Email'
  },

  payments: {
    both: 'Cả hai',
    atHotel: 'Tại khách sạn',
    online: 'Trả trước'
  },

  hotelTypes: {
    allTypes: 'Tất cả loại',
    none: '---',
    couples: 'Tình nhân',
    travel: 'Du lịch',
    couplesAndTravel: 'Tình nhân và Du lịch',
    quarantine: 'Cách ly'
  },

  contracts: {
    notClassified: 'None',
    type1: 'Type 1 - 600.000',
    type2: 'Type 2 - 300.000',
    type3: 'Type 3 - 100.000',
    type4Recontracted: 'Type 4 Recontracted',
    typeGo2joyRoom: 'Type Go2Joy Room'
  },

  roomType: {
    hourly: 'Theo giờ',
    additional: 'Giờ  thêm',
    overnight: 'Qua đêm',
    daily: 'Theo ngày'
  },

  dateOfWeek: {
    monday: 'Thứ hai',
    tuesday: 'Thứ ba',
    wednesday: 'Thứ tư',
    thursday: 'Thứ năm',
    friday: 'Thứ sáu',
    saturday: 'Thứ bảy',
    sunday: 'Chủ nhật'
  },

  // option trùng payment
  paymentsHotel: {
    both: 'Cả hai',
    payHotel: 'Tại Khách sạn',
    payOnline: 'Trả trước',
    momo: 'MoMo',
    credit: 'Thẻ tín dụng (OnePay)',
    debit: 'Thẻ ghi nợ (OnePay)',
    payoo: 'Pay@Store - Payoo',
    zalo: 'ZaloPay',
    pos: 'Pay@Store',
    creditCardVnpt: 'Thẻ tín dụng (VNPT)',
    atmCardVnpt: 'Thẻ ATM (VNPT)',
    shopeePay: 'ShopeePay'
  },

  status: {
    disable: 'Vô hiệu',
    deleted: 'Đã xóa',
    active: 'Hoạt động',
    expired: 'Hết hạn',
    temp: 'Nhiệt độ',
    draft: 'Lưu tạm',
    wait_confirm: 'Đợi xác nhận',
    get: 'Nhận',
    use: 'Sử dụng',
    not_yet: 'Chưa hoạt động',
    never: 'Không bao giờ',
    missing: 'Còn thiếu',
    no_need: 'Không cần',
    waiting: 'Chờ duyệt',
    available: 'Hoạt động',
    suspend: 'Hoãn',
    end: 'Kết thúc',

    allStatus: 'Tất cả trạng thái',
    created: 'Đã tạo',
    sending: 'Đang gửi',
    sent: 'Đã gửi',

    none: 'Không áp dụng',
    allUser: 'Tất cả User',
    allNoticeType: 'Tất cả loại thông báo',
    sendTo: 'Gửi đến',
    allImportFromExcelFile: 'Nhập từ excel file(.xslx)',
    userInput: 'Nhập thông tin người dùng',
    lastTimeOpenApp: 'Lần cuối mở app',
    go2joy: 'Go2Joy',
    allPartner: 'Tất cả Partner',
    booking: 'Đặt phòng',
    allTrailPartner: 'Tất cả Trial',
    allContractPartner: 'Tất cả Contract',
    hotel: 'Khách sạn',
    crmFilter: 'Bộ lộc CRM',
    normal: 'Thông dụng',
    donateCoupon: 'Tặng coupon',
    remindCoupon: 'Nhắc nhở'
  },
  applyHotelType: {
    applyAllHotel: 'Tất cả khách sạn',
    applyAllHotelExcept: 'Tất cả khách sạn ngọai trừ',
    onlyApply: 'Chỉ áp dụng'
  },
  applyUserType: {
    applyAllUser: 'Tất cả người dùng',
    applyAllUserExcept: 'Tất cả người dùng ngọai trừ',
    onlyApply: 'Chỉ áp dụng'
  },
  applyPromotionType: {
    applyAllPromo: 'Tất cả khuyến mãi',
    justApply: 'Chỉ áp dụng'
  },
  room: {
    normal: 'Normal',
    flash_sale: 'Flash sale'
  },
  typePromotion: {
    all: 'Tất cả',
    apply: 'Áp dụng',
    event: 'Sự kiện',
    inviteFriend: 'Mời bạn',
    gift: 'Quà tặng',
    booking: 'Đặt phòng',
    report_new_hotel: 'Báo khách sạn mới',
    sign_up: 'Đăng kí mới',
    birthday: 'Sinh nhật',
    payment: 'Thanh toán',
    voucher_code: 'Mã voucher'
  },

  discountType: {
    discountMoney: 'Giảm giá tiền',
    discount_percent: 'Giảm giá %',
    gift: 'Quà tặng',
    discountHours: 'Giảm giờ',
    samePrice: 'Đồng giá'
  },

  sendingStatus: {
    notSendYet: 'Không gửi được',
    error: 'Lỗi',
    ok: 'Thành công',
    dontSend: 'Không gửi'
  },

  targetType: {
    notice: 'Thông báo',
    promotion: 'Khuyến mãi',
    serviceAgreement: 'Thỏa thuận dịch vụ',
    hotel: 'Khách sạn',
    faq: 'FAQ',
    inviteFriend: 'Mời bạn bè',
    openApp: 'Mở ứng dụng',
    mileagePoint: 'Tích điểm',
    directDiscount: 'Giảm giá',
    signUp: 'Đăng ký',
    promotionGroup: 'Nhóm quảng cáo',
    amenityPack: 'Gói tiện ích',
    hotelGo2Choice: 'Khách sạn Go2Choice',
    linkOutApp: 'Link liên kết (Ngoài App)',
    linkInApp: 'Link liên kết (Trong App)',
    district: 'Quận',
    coupon: 'Coupon',
    promotionEvent: 'Khuyến mãi / Sự kiện',
    link: 'Link liên kết',
    hotelTet: 'Hotel Tet',
    referral: 'Chương trình giới thiệu',
    hotelArea: 'Khách sạn theo khu vực',
    hotelAdmin: 'Hình ảnh'
  },

  bookingStatus: {
    all: 'Tất cả',
    reserved: 'Đã đặt',
    confirmed: 'Chưa nhận',
    check_in: 'Đã nhận',
    cancel: 'Đã hủy',
    no_show: 'Không nhận phòng',
    payment_fail: 'Thanh toán thất bại'
  },
  confirmedBy: {
    user: 'Người dùng',
    hotel: 'Khách sạn',
    go2joy: 'Go2Joy',
    hotel_late: 'Khách sạn trễ',
    location: 'Vị trí'
  },
  noticeType: {
    function: 'Chức năng',
    notice: 'Tin nhắn',
    promotion: 'Khuyến mãi',
    term_policy: 'Quy định và chính sách',
    notification: 'Thông báo',
    notice_of_no_show: 'Thông báo Không nhận phòng',
    notice_of_Checked_in_l: 'Thông báo Nhận phòng L'
  },
  applyHotelOption: {
    all_hotel: 'Tất cả khách sạn',
    all_contracted: 'Tất cả ks hợp đồng',
    all_trial: 'Tất cả ks dùng thử ',
    just_apply: 'Chỉ áp dụng'
  },
  region: {
    all_region: 'Tất cả khu vực',
    south: 'Miền Nam',
    north: 'Miền Bắc',
    north_center: 'Trung bắc',
    south_center: 'Trung nam',
    middle: 'Miền Trung',
    confirm: 'Xác nhận',
    view: 'Xem'
  },

  type: {
    all_types: 'Tất cả',
    transfer_booking: 'Chuyển đổi đặt phòng',
    transfer: 'Chuyển đổi',
    promotion: 'Khuyến mãi',
    invite_friend: 'Mời bạn',
    product: 'Sản phẩm KS'
  },

  level: {
    level1: 'Cấp duyệt 1',
    level2: 'Cấp duyệt 2'
  },

  regionConfirm: {
    confirm: 'Xác nhận',
    view: 'Xem'
  },
  userConfirm: {
    Yes: 'Đã nhận',
    noShow: '',
    notYet: 'Chưa nhận'
  },
  hotelConfirm: {
    Yes: 'Đã nhận',
    noShow: 'Khách không đến',
    notYet: 'Chưa nhận'
  },
  all_rooms: 'Tất cả phòng',
  room_available: 'Còn phòng',
  room_unavailable: 'Hết phòng',

  window: 'Cửa Sổ',
  balcony: 'Ban Công',
  city: 'Thành Phố',
  garden: 'Sân Vườn',
  pool: 'Hồ Bơi',
  noView: 'Không Có',

  allSource: 'Tất cả nguồn',
  go2joy: 'Go2Joy',
  system: 'Hệ thống',
  agoda: 'Agoda',
  momo: 'MoMo',

  confirmed: 'Chưa nhận',
  noShowL: 'Không nhận phòng L',

  awaitingPay: 'Chờ thanh toán',
  awaitingConfirm: 'Chờ xác nhận phòng',
  upComing: 'Chờ nhận phòng',
  completedCheckedIn: 'Hoàn thành (Đã nhận)',
  checkedIn: 'Đã nhận phòng',
  cancelled: 'Đã hủy',
  noShow: 'Không nhận phòng',
  waitForGo2Joy: 'Chờ Go2Joy xử lý',
  complete: 'Hoàn thành',
  init: 'Khởi tạo',

  go2joyCancel: 'Go2Joy hủy',
  hotelCancel: 'Khách sạn hủy',
  userCancel: 'Khách hủy',

  hourly: 'Theo giờ',
  overnight: 'Qua đêm',
  daily: 'Theo ngày',

  payHotel: 'Tại khách sạn',
  payOnline: 'Trả online',

  user: 'Người dùng',
  hotel: 'Khách sạn',

  unpaid: 'Chưa thanh toán',
  paid: 'Đã thanh toán',
  refunded: 'Đã hoàn tiền',

  allDevice: 'Tất cả thiết bị',
  app: 'App',
  web: 'Web',
  checkInTime: 'Thời gian nhận phòng (Thời gian ở KS)',
  completedTime: 'Thời gian hoàn thành (Thời gian nhận phòng)',

  contracted: 'Hợp đồng',
  trial: 'Dùng thử',
  terminated: 'Dừng hợp đồng',
  suspended: 'Treo'
}
