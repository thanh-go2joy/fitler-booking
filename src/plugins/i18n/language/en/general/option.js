export default
{
  yes: 'Yes',
  no: 'No',
  all: 'All',
  selected: 'Selected',
  allSelected: 'All selected',
  public: 'Public',
  private: 'Private',
  hight: 'Hight',
  weight: 'weight',
  hotelStatus: {
    waiting: 'Waiting',
    displayed: 'Displayed',
    contracted: 'Contracted',
    trial: 'Trial',
    terminated: 'Terminated',
    suspended: 'Suspended',

    hotelAvailable: 'Available',
    hotelUnAvailable: 'Unavailable'
  },

  bookingNotificationBy: {
    go2joyParterApp: 'Go2Joy\'s Partner App',
    sms: 'SMS',
    email: 'Email'
  },

  payments: {
    both: 'Both',
    atHotel: 'At hotel',
    online: 'Just pay online'
  },

  hotelTypes: {
    allTypes: 'All types',
    none: 'None',
    couples: 'Couples',
    travel: 'Travel',
    couplesAndTravel: 'Couples and Travel',
    quarantine: 'Quarantine'
  },

  contracts: {
    notClassified: 'None',
    type1: 'Type 1 - 600.000',
    type2: 'Type 2 - 300.000',
    type3: 'Type 3 - 100.000',
    type4Recontracted: 'Type 4 Recontracted',
    typeGo2joyRoom: 'Type Go2Joy Room'
  },

  roomType: {
    hourly: 'Hourly',
    additional: 'Additional',
    overnight: 'Overnight',
    daily: 'Daily'
  },

  dateOfWeek: {
    monday: 'Monday',
    tuesday: 'Tuesday',
    wednesday: 'Wednesday',
    thursday: 'Thursday',
    friday: 'Friday',
    saturday: 'Saturday',
    sunday: 'Sunday'
  },

  // option trùng payment
  paymentsHotel: {
    both: 'Both',
    payHotel: 'Pay at hotel',
    payOnline: 'Just pay online',
    momo: 'MoMo',
    credit: 'Credit card (OnePay)',
    debit: 'Debit card (OnePay)',
    payoo: 'Pay@Store - Payoo',
    zalo: 'ZaloPay',
    pos: 'Pay@Store',
    creditCardVnpt: 'Credit card (VNPT)',
    atmCardVnpt: 'ATM card (VNPT)',
    shopeePay: 'ShopeePay'
  },

  status: {
    disable: 'Disable',
    deleted: 'Deleted',
    active: 'Active',
    expired: 'Expired',
    temp: 'Temp',
    draft: 'Draft',
    wait_confirm: 'Wait confirm',
    get: 'Get',
    use: 'Use',
    not_yet: 'Not yet',
    never: 'Never',
    missing: 'Missing',
    no_need: 'No need',
    waiting: 'Waiting',
    available: 'Available',
    suspend: 'Suspend',
    end: 'End',

    allStatus: 'All status',
    created: 'Created',
    sending: 'Sending',
    sent: 'Sent',

    none: 'None',
    allUser: 'All User',
    allNoticeType: 'All notice type',
    sendTo: 'Send to',
    allImportFromExcelFile: 'All Import From Excel File',
    userInput: 'User Input',
    lastTimeOpenApp: 'Last time open app',
    go2joy: 'Go2Joy',
    allPartner: 'All Partner',
    booking: 'Booking',
    allTrailPartner: 'All Trail Partner',
    allContractPartner: 'All Contract Partner',
    hotel: 'Hotel',
    crmFilter: 'CRM Filter',
    normal: 'Normal',
    donateCoupon: 'Donate Coupon',
    remindCoupon: 'Remind Coupon'
  },

  applyHotelType: {
    applyAllHotel: 'Apply all hotel',
    applyAllHotelExcept: 'Apply all hotel except',
    onlyApply: 'Only apply'
  },
  applyUserType: {
    applyAllUser: 'Apply all user',
    applyAllUserExcept: 'Apply all user except',
    onlyApply: 'Only apply'
  },
  applyPromotionType: {
    applyAllPromo: 'Apply all promotion',
    justApply: 'Just apply'
  },
  room: {
    normal: 'Normal',
    flash_sale: 'Flash sale'
  },
  typePromotion: {
    all: 'All',
    apply: 'Apply',
    event: 'Event',
    inviteFriend: 'Invite friend',
    gift: 'Gift',
    booking: 'Booking',
    report_new_hotel: 'Report new hotel',
    sign_up: 'Sign up',
    birthday: 'Birthday',
    payment: 'Payment',
    voucher_code: 'Voucher code'
  },
  discountType: {
    discountMoney: 'Discount money',
    discount_percent: 'Discount %',
    gift: 'Gift',
    discountHours: 'Discount hours',
    samePrice: 'Same price'
  },

  sendingStatus: {
    notSendYet: 'Not send yet',
    error: 'Error',
    ok: 'Ok',
    dontSend: "Don't send"
  },
  bookingStatus: {
    all: 'All',
    reserved: 'Reserved',
    confirmed: 'Confirmed',
    check_in: 'Checked-in',
    cancel: 'Cancelled',
    no_show: 'No show',
    payment_fail: 'Payment Fail'
  },
  confirmedBy: {
    user: 'User',
    hotel: 'Hotel',
    go2joy: 'Go2Joy',
    hotel_late: 'Hotel late',
    location: 'Location'
  },

  targetType: {
    notice: 'Notice',
    promotion: 'Promotion',
    serviceAgreement: 'Service Agreement',
    hotel: 'Hotel',
    faq: 'FAQ',
    inviteFriend: 'Invite Friend',
    openApp: 'Open app',
    mileagePoint: 'Mileage Point',
    directDiscount: 'Direct Discount',
    signUp: 'Sign up',
    promotionGroup: 'Promotion Group',
    amenityPack: 'Amenities pack',
    hotelGo2Choice: 'Hotel Go2Choice',
    district: 'District',
    promotionEvent: 'Promotion / Event',
    linkOutApp: 'Link (Out-App)',
    linkInApp: 'Link (In-app)',
    coupon: 'Coupon',
    link: 'Link',
    hotelTet: 'Hotel Tet',
    referral: 'Referral program',
    hotelArea: 'Hotel Area',
    hotelAdmin: 'Image'
  },

  noticeType: {
    function: 'Function',
    promotion: 'Promotion',
    term_policy: 'Term & Policy',
    notification: 'Notification',
    notice_of_no_show: 'Notice of No Show',
    notice_of_Checked_in_l: 'Notice of Checked-in L'
  },

  applyHotelOption: {
    all_hotel: 'All hotels',
    all_contracted: 'All hotels contracted',
    all_trial: 'All hotels trial',
    just_apply: 'Just apply',
    hotelGo2Choice: 'Hotel Go2Choice',
    link: 'Link',
    district: 'District',
    coupon: 'Coupon',
    promotionEvent: 'Promotion / Event'
  },

  region: {
    all_region: 'All region',
    south: 'South',
    north: 'North',
    north_center: 'North center',
    south_center: 'South center',
    middle: 'Middle',
    confirm: 'Confirm',
    view: 'View'
  },

  type: {
    all_types: 'All types',
    transfer_booking: 'Transfer Booking',
    transfer: 'Transfer',
    promotion: 'Promotion',
    invite_friend: 'Invite friend',
    product: 'Product'
  },

  level: {
    level1: 'Level 1',
    level2: 'Level 2'
  },

  regionConfirm: {
    confirm: 'Confirm',
    view: 'View'
  },
  userConfirm: {
    Yes: 'Yes',
    notYet: 'not yet',
    noShow: 'No show'
  },
  hotelConfirm: {
    Yes: 'Yes',
    notYet: 'not yet',
    noShow: 'No show'
  },
  all_rooms: 'All rooms',
  room_available: 'Room available',
  room_unavailable: 'Room unavailable',

  window: 'Window',
  balcony: 'Balcony',
  city: 'City',
  garden: 'Garden',
  pool: 'Pool',
  noView: 'No view',

  allSource: 'All source',
  go2joy: 'Go2Joy',
  system: 'System',
  agoda: 'Agoda',
  momo: 'MoMo',

  confirmed: 'Confirmed',
  noShowL: 'No show L',

  awaitingPay: 'Wait for payment',
  awaitingConfirm: 'Wait for confirmation',
  upComing: 'Upcoming',
  completedCheckedIn: 'Completed (Checked-in)',
  checkedIn: 'Checked-in',
  cancelled: 'Cancelled',
  noShow: 'No show',
  waitForGo2Joy: 'Wait for Go2Joy',
  complete: 'Completed',
  init: 'Initialization',

  go2joyCancel: 'Go2Joy cancel',
  hotelCancel: 'Hotel cancel',
  userCancel: 'User cancel',

  hourly: 'Hourly',
  overnight: 'Overnight',
  daily: 'Daily',

  payHotel: 'Pay at hotel',
  payOnline: 'Just pay online',

  user: 'User',
  hotel: 'Hotel',

  unpaid: 'Unpaid',
  paid: 'Paid',
  refunded: 'Refunded',

  allDevice: 'All device',
  app: 'App',
  web: 'Web',
  checkInTime: 'Check-in time (Stay at hotel time)',
  completedTime: 'Completed time (Check-in time)',

  contracted: 'Contracted',
  trial: 'Trial',
  terminated: 'Terminated',
  suspended: 'Suspended'
}
